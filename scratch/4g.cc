...
int main (int argc, char *argv[])
{
  ...
  uint32_t numerology=2 ; // the numerology

  // applications
  uint32_t packetSize = 1024; // UDP packet size in bytes
  uint32_t startTime = 0.5; // application start time in milliseconds
  uint32_t endTime = 10; // application end time in milliseconds
  ...
  Simulator::Stop (MilliSeconds (endTime + 10));
  ...
  outdata.open("RPresult4g.csv", std::ofstream::app); // opens the file
   if( !outdata ) { // file couldn't be opened
      cerr << "Error: file could not be opened" << endl;
      exit(1);
   }
  outdata << g_rxPackets<<","<<throughput<<","<<humidity<<","<<visibility<<","<<particleradius<<","<<frequency<<","<<g_firstReceived.GetSeconds()<<","<<g_lastReceived.GetSeconds()<<","<<intraGroupDistance << endl;
  outdata.close();
